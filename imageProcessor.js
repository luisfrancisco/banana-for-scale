...
async function processImages(req, res) {
  try {
    ...
    const resizedProductImageBuffer = await uploadedImage
      .resize({
    ...
    const resizedProductImageMetadata = await sharp(
      resizedProductImageBuffer
    ).metadata();
    ...
    const outputBuffer = await referenceImage
      .composite([
        {
    ...
    const outputWithText = await drawTextOnImage(
      outputBuffer,
      `${req.body.productHeight} cm`,
      leftPosition,
      topPosition
    );
